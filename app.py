...
def run_simulation(params):
    ...
    def save_plot1(): fig, ax = plt.subplots(figsize=(10,4)); ax.plot(df1['datetime'], df1['solar_flux']); ax.set_xlabel('Дата'); ax.set_ylabel('Solar flux (W/m²)'); ax.set_title('Интенсивность солнечного потока (Clear Sky)'); ax.grid(True); fig.tight_layout(); fig.savefig(os.path.join(img_dir, 'solar_flux_clear.png')); plt.close(fig)
    ...
    def save_plot4(): fig, ax = plt.subplots(figsize=(10,4)); ax.plot(df3['datetime'], df3['solar_flux']); ax.set_xlabel('Дата'); ax.set_ylabel('Solar flux (W/m²)'); ax.set_title('Интенсивность солнечного потока с учётом облачности'); ax.grid(True); fig.tight_layout(); fig.savefig(os.path.join(img_dir, 'solar_flux_cloudy.png')); plt.close(fig)
